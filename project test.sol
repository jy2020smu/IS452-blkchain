...
contract HDBRentalContract {
	...
	struct HDB {
		...
		uint rentAmount; // HDB rent value
...
// Payment
	struct MonthlyRentStatus {
		uint amount; // Monthly rent amount to be paid (includes utilities)
...
// Rental status
 ...
 // constructor to initialise the contract
	...
	function warnTenant(string calldata month) external onlyOwner allowedMonths(month) returns (bool success) {
		...
		if((rentInStore[month].status == false) && (now > hdb.dueDate) && ((now - warningTime) > 172800000)) {
			hdb.tenantWarning++;
			warningTime = now;
